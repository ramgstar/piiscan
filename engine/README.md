...
[piiscan](../README.md)의 정규식 매칭 단계로, Rust로 작성됐습니다. 컬럼 값 배치를 JSONL로
읽어들여 컴파일된 패턴 집합을 적용하고, 정규식 **후보**를 모두 JSONL로 다시 써냅니다. 정규식
매칭만 담당하며 — 체크섬 검증은 Java analyzer에서 이뤄집니다. analyzer는 이 바이너리를 배치당
한 번씩 자식 프로세스로 실행합니다.
...
analyzer가 배치별 실패 유형을 구분할 수 있도록 안정적으로 유지됩니다:
